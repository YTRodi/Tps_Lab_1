...
#include "parser.h" // ULTIMO AGREGADOOOOOOOOOOOOOOOO----------------
...
int parser_EmployeeFromText(FILE* pFile, LinkedList* pArrayListEmployee)
{
    int retorno = -1;
    char buffer[4][30];
    int cant;//cantidad de variables que nos pudo retornar.
    Employee* auxEmple;

    fscanf(pFile,"%[^,],%[^,],%[^,],%[^\n]\n",buffer[0],buffer[1],buffer[2],buffer[3]);//con esto borro el archivo de cabecera
